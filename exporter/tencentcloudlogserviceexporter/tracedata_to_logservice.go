...
func traceDataToLogServiceData(td ptrace.Traces) []*clssdk.Log {
	var clsLogs []*clssdk.Log
	resourceSpansSlice := td.ResourceSpans()
	for i := 0; i < resourceSpansSlice.Len(); i++ {
		logs := resourceSpansToLogServiceData(resourceSpansSlice.At(i))
		clsLogs = append(clsLogs, logs...)
	}
	return clsLogs
}

func resourceSpansToLogServiceData(resourceSpans ptrace.ResourceSpans) []*clssdk.Log {
	...
	for i := 0; i < scopeSpansSlice.Len(); i++ {
		...
		instrumentationLibraryContents := instrumentationLibraryToLogContents(insLibSpans.Scope())
		spans := insLibSpans.Spans()
		for j := 0; j < spans.Len(); j++ {
			if clsLog := spanToLogServiceData(spans.At(j), resourceContents, instrumentationLibraryContents); clsLog != nil {
...
func spanToLogServiceData(span ptrace.Span, resourceContents, instrumentationLibraryContents []*clssdk.Log_Content) *clssdk.Log {
	timeNano := int64(span.EndTimestamp())
	if timeNano == 0 {
		timeNano = time.Now().UnixNano()
	}
	clsLog := clssdk.Log{
		Time: proto.Int64(timeNano / 1000 / 1000),
	}
	...
	contentsBuffer := make([]clssdk.Log_Content, 0, preAllocCount)
	...
	for i := range contentsBuffer {
		clsLog.Contents = append(clsLog.Contents, &contentsBuffer[i])
	}
	return &clsLog
}
