...
* The `routing_key` property is used to specify how to route values (spans, logs, or metrics) to exporters based on different parameters. It supports one of the following values:
  ...
  * `attributes`: Routes based on values in attributes. For traces, resource, scope, and span attributes are checked, with `span.kind` and `span.name` as pseudo attributes. For logs, resource, scope, and log record attributes are checked, with `log.severity` and `log.body` as pseudo attributes. For metrics, resource / scope / datapoint attributes are supported. Useful for situations in which a single service overwhelms any given instance of the collector, and should be split over multiple collectors.
  * `traceID`: Routes based on `traceID`. Works for traces and logs. For logs, each log record's trace ID is used as the routing key; logs without a trace ID are routed together. Invalid for metrics.
  * `resource`: Routes based on a hash of all resource attributes. Works for logs and metrics. Useful when you need finer-grained routing than `service` but don't want to specify individual attributes.
