...
func TestLogsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			allEventsCount++
			lb.RecordDbServerTopQueryEvent(ctx, timestamp, 27.100000, "db.query.text-val", 25, 29, 30, 30, "sqlserver.query_hash-val", "sqlserver.query_plan-val", "sqlserver.query_plan_hash-val", 20, 28.100000, 24, "server.address-val", 11, "db.system.name-val", 35, "sqlserver.procedure_id-val", "sqlserver.procedure_name-val")
			...
			for i := 0; i < lrs.Len(); i++ {
				...
				case "db.server.query_sample":
					...
					attrVal, ok = lr.Attributes().Get("sqlserver.wait.resource.id")
					assert.True(t, ok)
					assert.Equal(t, "sqlserver.wait.resource.id-val", attrVal.Str())
					attrVal, ok = lr.Attributes().Get("sqlserver.wait.resource.type")
					assert.True(t, ok)
					assert.Equal(t, "sqlserver.wait.resource.type-val", attrVal.Str())
